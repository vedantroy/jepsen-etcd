...
(defn sample-generator-ops!
  ...
  (let [ctx (gen/context test)]
    (loop [ops []]
      (if (= n (count ops))
        ops
        (let [op (gen/op generator test ctx)]
          (cond
            (= :pending op)
            (throw (ex-info "Generator returned :pending during lock-set sampling"
                            {:op-count (count ops)}))

            (nil? op)
            (throw (ex-info "Generator ended unexpectedly during lock-set sampling"
                            {:op-count (count ops)}))
...
(defn validate-lock-set-generator!
  ...
  (let [workload (lock/set-workload sample-opts)
        ...
        test (assoc sample-opts
                    :generator generator
                    :pure-generators true)
        ops (sample-generator-ops! generator test 100)
        fs (set (map :f ops))]
    (when-not (every? lock-set-op? ops)
      (throw (ex-info "Invalid lock-set generator op"
                      {:ops (remove lock-set-op? ops)})))
    (when-not (= #{:add :read} fs)
      (throw (ex-info "Lock-set generator sample did not include both operation types"
                      {:fns fs})))
    (println "OK lock-set generator sample" (count ops) "ops")))
